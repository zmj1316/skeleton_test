...
namespace vmd
{
	...
	public:
		...
		void Read(std::istream* stream)
		{
			char buffer[15];
			stream->read((char*) &buffer, sizeof(char) * 15);
			face_name = oguna::EncodingConverter::sjis2utf16(buffer);
			stream->read((char*) &frame, sizeof(int));
	...
	public:
		...
		void Read(std::istream *stream)
		{
			...
			for (int i = 0; i < ik_count; i++)
			{
				...
				ik_enable[i].ik_name = oguna::EncodingConverter::sjis2utf16(buffer);
	...
	public:
		...
		static std::unique_ptr<VmdMotion> LoadFromStream(std::ifstream *stream)
		{
			...
			result->model_name = oguna::EncodingConverter::sjis2utf16(buffer);
